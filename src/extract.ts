...
export async function extractTarGz(archivePath: string, outputDir: string): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    const proc = spawn(tarCmd, tarArgs, { stdio: ['ignore', 'ignore', 'ignore'] })

    proc.on('close', (code) => {
      if (code === 0) {
        resolve()
      } else {
        // Simple user-friendly error message regardless of platform
        reject(new Error('Failed to extract tar.gz archive'))
      }
    })
