...
import { findBestAsset, getPlatformInfo, matchAssetName } from './platform.js';
...
describe('Platform Detection', () => {
  ...
  test('matchAssetName detects macOS assets correctly', () => {
    ...
    assert.equal(
      matchAssetName('app-v1.0.0-x86_64-apple-darwin.tar.gz', platform),
      true,
    );
    assert.equal(matchAssetName('app-v1.0.0-amd64-macos.zip', platform), true);
    assert.equal(
      matchAssetName('app-v1.0.0-x64-darwin.tar.gz', platform),
      true,
    );
    assert.equal(
      matchAssetName('app-v1.0.0-x86_64-apple.tar.gz', platform),
      true,
    );
  });
  ...
  test('matchAssetName detects Linux assets correctly', () => {
    ...
    assert.equal(
      matchAssetName('app-v1.0.0-x86_64-unknown-linux-gnu.tar.gz', platform),
      true,
    );
    assert.equal(
      matchAssetName('app-v1.0.0-amd64-linux.tar.gz', platform),
      true,
    );
    assert.equal(
      matchAssetName('app-v1.0.0-x64-linux-gnu.tar.gz', platform),
      true,
    );
  });
  ...
  test('matchAssetName detects Windows assets correctly', () => {
    ...
    assert.equal(
      matchAssetName('app-v1.0.0-x86_64-pc-windows-msvc.zip', platform),
      true,
    );
    assert.equal(
      matchAssetName('app-v1.0.0-amd64-windows.zip', platform),
      true,
    );
  ...
  test('matchAssetName detects ARM64 assets correctly', () => {
    ...
    assert.equal(
      matchAssetName('app-v1.0.0-aarch64-apple-darwin.tar.gz', platform),
      true,
    );
    assert.equal(
      matchAssetName('app-v1.0.0-arm64-macos.tar.gz', platform),
      true,
    );
  });
  ...
  test('findBestAsset prefers tar.gz over zip on non-Windows', () => {
    ...
    const assets = [
      'app-v1.0.0-x86_64-apple-darwin.zip',
      'app-v1.0.0-x86_64-apple-darwin.tar.gz',
    ];
  ...
  test('findBestAsset returns first match when no tar.gz preference', () => {
    const platform = { platform: 'win32', arch: 'x64', combined: 'win32-x64' };
    const assets = [
      'app-v1.0.0-x86_64-pc-windows-msvc.zip',
      'app-v1.0.0-x64-windows.zip',
    ];
